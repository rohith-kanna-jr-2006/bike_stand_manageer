...
import { UserRole, AuthProvider, PaymentMethod } from '../types';
...
export const SettingsPage: React.FC = () => {
  ...
  const [twoFaStep, setTwoFaStep] = useState<'select' | 'setup' | 'verify'>('select');
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      if (user) {
        ...
        // await userService.updateProfile(user.id, { name, phoneNumber: phone, address, avatar: avatarPreview, paymentMethods: cards });

        // Call backend API to update user profile
        await fetch(`http://localhost:3002/api/users/${user.id}`, {
          method: 'PUT',
          headers: {
  ...
  const savePaymentMethodsToFirestore = async (newCards: PaymentMethod[]) => {
    setCards(newCards);
  ...
  const getDetectedCardType = (number: string) => {
  ...
  const handle2FAMethodSelect = async (method: 'app' | 'sms') => {
    if (method === 'sms') {
      if (!phone) {
