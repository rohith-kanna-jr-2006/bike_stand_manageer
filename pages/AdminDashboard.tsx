...
import { Stand, Booking } from '../types';
import {
...
interface AdminDashboardProps {
    onNavigate?: (view: 'dashboard' | 'settings' | 'add-stand') => void;
}
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate }) => {
    ...
    const [utilizationChartData, setUtilizationChartData] = useState<{ name: string, value: number }[]>([]);
    ...
    const fetchStands = async () => {
        ...
        try {
            ...
            const standsData = response.data.map((s: any) => ({
                ...s,
                id: s._id,
                location: {
            ...
            const activeStands = standsData.filter((s: any) => s.status === 'active');
            const maintenanceStands = standsData.filter((s: any) => s.status === 'maintenance');

            const occupied = activeStands.reduce((acc: number, s: any) => acc + (s.capacity - s.availableSpots), 0);
            const available = activeStands.reduce((acc: number, s: any) => acc + s.availableSpots, 0);
            const maintenance = maintenanceStands.reduce((acc: number, s: any) => acc + s.capacity, 0);
    ...
    const fetchBookings = async () => {
        if (!user) return;
        try {
            // Fetch bookings for all stands owned by the user
            // Ideally backend should support filtering bookings by ownerId directly or we iterate stands
            // For now, let's assume we fetch all bookings for the user's stands.
            // Since our API is by stand or by user, we might need a new endpoint or iterate.
            // But wait, the backend `getStandBookings` gets bookings for ONE stand.
            // We need an endpoint to get ALL bookings for an ADMIN (owner).
            // Let's assume for now we iterate over stands or add a new endpoint later.
            // Actually, let's iterate for now as a temporary solution.

            // Better approach: The backend should have an endpoint for this.
            // But to keep it simple and working with current backend:
            const allBookings: Booking[] = [];
            // We need stands first.
            // Let's rely on `stands` state or fetch again.
            // Since `fetchStands` runs on mount, `stands` might not be ready here immediately if we don't chain.

            // Let's fetch stands here again to be sure or wait for stands state?
            // A better way is to have a specific endpoint.
            // Let's skip complex logic and just fetch for the first stand for demo or implement a loop.

            // Wait, I can update the backend to support getting all bookings for an owner.
            // But I cannot change backend right now easily without context switching.
            // Let's try to fetch bookings for each stand.
    ...
    const [viewBookingsStand, setViewBookingsStand] = useState<Stand | null>(null);

    // Form State
    const [newStandName, setNewStandName] = useState('');
    const [newStandCapacity, setNewStandCapacity] = useState('20');
    const [newStandLoc, setNewStandLoc] = useState<{ lat: number, lng: number } | null>(null);
    ...
    const [newStandRates, setNewStandRates] = useState({ bike: '20', car: '50' });
    ...
    const handleEditStandClick = (stand: Stand) => {
        ...
        setNewStandRates({
            bike: stand.rates?.bike.toString() || '20',
    ...
    const handleViewBookings = (stand: Stand) => {
    ...
    const handleBulkStatusUpdate = async (status: 'active' | 'maintenance') => {
    ...
    const handleOpenAddModal = () => {
        ...
        setNewStandEmail('');
        setNewStandLicense('');
        setNewStandRates({ bike: '20', car: '50' });
        setShowStandModal(true);
    };
    ...
    useEffect(() => {
        const initMonitorMap = async () => {
            if (viewMode === 'map' && mapRef.current) {
                if (!googleMapInstance.current && typeof google !== 'undefined') {
                    try {
                        ...
                        // Fallback handling if map fails to load is handled by state/ref checks
                        return;
                    }
                }
                ...
                if (googleMapInstance.current) {
                    ...
                    filteredStands.forEach(stand => {
                        ...
                        marker.addListener("click", () => {
                            // Create InfoWindow Content Programmatically with interactive buttons
    ...
    const handleSaveStand = async () => {
        ...
        const standData = {
            ...
            status: 'active',
            ownerId: user.id, // CRITICAL: This assigns ownership to the current Admin
            contact: {
                phone: newStandPhone,
                email: newStandEmail,
                                            ...
                                            <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center text-gray-900">
                                                        ...
                                                        {booking.startTime?.toDate ? booking.startTime.toDate().toLocaleDateString() : new Date(booking.startTime).toLocaleDateString()}
                                                    </div>
                                                    <div className="flex items-center text-xs text-gray-500 mt-1 ml-5.5 pl-0.5">
                                                        <Clock className="h-3 w-3 mr-1.5" />
                                                        {booking.startTime?.toDate ? booking.startTime.toDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : new Date(booking.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                    </div>
                                                </td>
                ...
                <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center space-x-4">
                        ...
                        <div className="flex bg-gray-100 p-1 rounded-lg">
                            ...
                            >
                                <div className="flex items-center"><Map className="h-3 w-3 mr-1" /> Map</div>
                            </button>
                            <button
